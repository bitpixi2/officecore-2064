# Encounter 001: Bushfire Edge Cache And The Great Servo Sausage Roll Incident
...
The campervan crew must keep people connected while moving through smoke, heat, failing cooling, road closures, awkward corporate PR, and a cascade of ridiculous civic mistakes.

This is still a bushfire encounter. Keep people and place safe enough to matter. The comedy comes from systems, office rituals, weird bodies, mismatched priorities, and everyone being competent in the most undignified possible way.
...
- Which random species/kind helper has already climbed into the wrong van?

## Randomness Dial

Use a six-sided die whenever the encounter feels too sensible, too grim, or too linear.

- Roll once at the start of each new location.
- Roll after any `6 or less`.
- Roll when Rooby posts.
- Roll when players ask "what else is here?"

The result should create a tool, problem, witness, joke, or social complication. It should not erase the fire stakes.

## Fast Weirdness Table

Roll `1d6`.

1. A clipboard has become legally powerful because everyone signed it without reading.
2. A public address system starts reading calendar invites instead of evacuation updates.
3. Someone's van module unfolds into a service counter and people immediately queue.
4. Rooby auto-tags the wrong expert, who is somehow perfect for the job and furious about it.
5. A species/kind NPC arrives with exactly the wrong vibe and exactly the right tool.
6. The problem can only be solved if someone gives it a temporary department name.

## Species/Kind NPC Table

Roll `1d6` or pick the funniest useful helper.

1. `Roo-Kin courier`: wearing sixteen lanyards, carrying a thermal camera, and refusing to explain the shortcut.
2. `Brush-Tail Night Clerk`: awake since Tuesday and guarding a milk crate of charged power banks with intense spreadsheet energy.
3. `Magpie Protocol influencer`: collecting official-looking badges and accidentally improving public trust.
4. `Civic Daemon temp`: projected from a tablet on a selfie stick, already has the incident number nobody else can find.
5. `Myco-Union Rep`: knows which wall is damp, which fridge is failing, and which room has been lying.
6. `Receipt Revenant`: holds the original warranty docket for the data-centre cooling unit and wants closure.

## Servo Chaos Table

Roll `1d6` at the Servo Command Post.

1. The hot-food display is now the warmest reliable meeting room.
2. The receipt printer produces a perfect evacuation route, but only on receipts for pineapple juice.
3. A council volunteer has made badges reading `TEMPORARY DEPUTY VIBES OFFICER`.
4. A fridge full of energy drinks is beeping in Morse code and nobody likes what it says.
5. The queue splits into `people needing help`, `people offering help`, and `people flirting with logistics`.
6. A child has drawn a better map than the official one, but labelled the data centre `THE BIG HUM`.

## Data-Centre Weirdness Table

Roll `1d6` at the Edge Data-Centre Perimeter.

1. The access control system accepts apology emails as credentials.
2. The cooling dashboard displays everything as weather icons and one of them is `sweaty filing cabinet`.
3. Corporate comms wants a photo of "community resilience" and accidentally blocks the fire lane.
4. The server room has a laminated sign: `NO ROMANCE NEAR RACK 4 AFTER THE INCIDENT`.
5. A rack named `Kelvin` keeps requesting a union representative through the monitoring logs.
6. The backup generator will start only after someone completes a satisfaction survey.

## Rooby Alert Table

Roll `1d6` when Rooby posts.
...
A service station carpark has become an accidental civic desk. There are folding tables, extension leads, handwritten lists, two firefighters, three people arguing about maps, one teenager calmly labelling water bottles, and a queue forming behind any person who looks like they own a stamp.
...
Silly lever: the servo keeps generating fake authority. Badges, receipts, PA announcements, queue numbers, and meal deals can all become tools.
...
Silly lever: road signs, outdated maps, and confident locals disagree in useful ways. The wrong route might be safer, flirtier, or full of snacks.
...
Silly lever: the data centre is too clean to admit it is panicking. Make every dashboard, badge, cooling duct, and policy sign reveal emotional damage.
...
Silly lever: the relay is physically embarrassing. It may be mounted on a novelty sign, a water tank shaped like a giant office mug, or a council shed with a mural of a fax machine.
...
Silly lever: every van has one oddly perfect feature and one humiliating limitation. Ask both before a roll.
...
Complication: one of them is live-rating the crew's emergency office layout.
...
Complication: Rooby can route help faster than anyone can socially process it.

### The Snack Table

Not a person, technically, but everyone treats it like a faction. It contains servo pies, emergency fruit, one suspiciously elegant thermos, and the last packet of decent biscuits.

Want: to become the emotional centre of the incident.

Useful offer: morale, bait for volunteers, an excuse to gather NPCs.

Complication: whoever controls the snack table is assumed to be in charge.
...
- `Snack Table Becomes Government`
...
The emergency relay is dropping packets, duplicating stale map updates, and translating every third location into a food item.
...
Random kicker: roll on the `Rooby Alert Table`.
...
Random kicker: roll on the `Data-Centre Weirdness Table`.
...
Random kicker: a species/kind NPC tries to help by impersonating an office role that should not exist.
...
Random kicker: one passenger has a highly specific demand about seating, snacks, music, or whether their emotional support printer can come too.
...
Random kicker: the cache needs a public-facing name. The worst suggestion is currently winning.
...
Random kicker: Rooby posts a poll with options that are all technically bad.

### Stop The Snack Table Coup

The snack table has become the most trusted institution on site. This is good for morale and bad for governance.

Likely rolls: `Charm`, `Grit`, `Spark`

Costs:

- someone must become Snack Secretary
- the best biscuits are spent as diplomatic currency
- an NPC develops a crush on the person with the thermos

Success means: gain `+1 Trust`, move one crowd to the right task, and stop the queue from blocking emergency access.
...
## Species/Kind Spotlights

- `Human Local`: knows the obvious practical thing everyone is too dramatic to try.
- `Roo-Kin`: reaches the relay, fence gap, or high shelf before the argument catches up.
- `Brush-Tail Night Clerk`: heard the private conversation because nobody checked the roof storage.
- `Magpie Protocol`: borrows a tiny authority signal and makes it useful.
- `Civic Daemon`: has a timestamp, ticket, or reminder from before the scene began.
- `Myco-Union Rep`: reads the building through damp, smell, and old leaks.
- `Receipt Revenant`: produces proof that this exact failure has happened before.
...
- Weird authority: temporary title such as Snack Secretary, Deputy Vibes Officer, Acting Relay Registrar, or Minister for Incorrect Maps.
